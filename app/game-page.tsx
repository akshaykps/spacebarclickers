...
export default function SpacebarClickerGame() {
  ...
  const scoreSubmitIntervalRef = useRef<NodeJS.Timeout | null>(null) // New ref for score submission
  ...
  useEffect(() => {
    const data = {
      ...
      clickMultiplier,
      achievements,
      username: currentUser?.username || '', // Save username from currentUser
      gameStats,
      soundEnabled,
      currentTheme,
    }
  ...
  // Score submission effect
  useEffect(() => {
    // Submit score every 30 seconds
    scoreSubmitIntervalRef.current = setInterval(() => {
      submitGameScore()
    }, 30000) // Submit every 30 seconds

    // Submit score on component unmount or page close
    ...
    return () => {
      if (scoreSubmitIntervalRef.current) {
        clearInterval(scoreSubmitIntervalRef.current)
      }
      window.removeEventListener('beforeunload', handleBeforeUnload)
    }
  }, [currentUser, totalClicks, gameStats, achievements, userCountryInfo]) // Dependencies for score submission
  ...
  const submitGameScore = useCallback(async () => {
    const currentUsername = currentUser?.username || 'Anonymous' // Use logged-in username or 'Anonymous'
    if (!currentUsername || totalClicks === 0) {
      console.log('Skipping score submission: username not set or no clicks.')
      return
    }
  ...
  }, [currentUser, totalClicks, gameStats, userCountryInfo])
          ...
          <div className='container mx-auto px-4 py-4'>
            <div className='flex justify-between items-center'>
              ...
              <div className='flex items-center gap-2'>
                ...
                {currentUser ? (
                  // If logged in, show settings and logout
                  <>
                    ...
                    <div className='mt-6 space-y-4'>
                      ...
                      <div className='space-y-4'>
                        <div>
                          ...
                          <Input
                            value={currentUser?.username || 'Anonymous'}
                            disabled // Username is from backend, not editable here
                            className='bg-slate-800 border-slate-600 text-white'
                          />
                        </div>
